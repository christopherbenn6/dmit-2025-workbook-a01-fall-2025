...
/**
 * Prepares and executes a statement. Requires: 
 * @param string - $query
 * @param array - $params
 * @param string - $types
 * @return mixed - Result set of SELECT queries or TRUE / FALSE 
 */
function execute_prepared_statement($query, $params = [], $types = "") {
    ...
    if(!$statement) {
        die("Preparaion failed: " . $connection->error);
    }

    // If we need to bind any parameters (i.e. edit, add, delete)
    if(!empty($params)) {
        $statement->bind_param($types, ...$params);
    }

    // This executes the statement within the if
...
/**
 * Retrieves all cities with SELECT
 */
